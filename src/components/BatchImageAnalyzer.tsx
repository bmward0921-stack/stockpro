...
import { Camera, Upload, Loader2, Scan, X, Library, Images, Plus, Trash2 } from 'lucide-react';
...
const BatchImageAnalyzer = ({ onProductsDetected }: BatchImageAnalyzerProps) => {
  ...
  const analyzeImage = async (image: CapturedImage): Promise<ProductDetails | null> => {
    try {
      const { data, error } = await supabase.functions.invoke('analyze-product-image', {
        body: { imageUrl: image.dataUrl },
      });
  ...
  const analyzeAllImages = async () => {
    ...
    for (let i = 0; i < capturedImages.length; i++) {
      ...
      try {
        const product = await analyzeImage(image);
            ...
            <div className="aspect-video w-full overflow-hidden rounded-lg bg-muted flex flex-col items-center justify-center gap-4 p-6 text-center">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground/75 mt-1">
                  Capture or upload multiple product images to analyze them all at once
                </p>
              </div>
            </div>
          )}
